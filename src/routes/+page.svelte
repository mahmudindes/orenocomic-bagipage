...
<svelte:head>
	<title>BagiDono - Comic Website Catalog</title>
</svelte:head>

<div class="flex h-72 items-center justify-center bg-slate-700 text-white">
	<h1 class="text-center text-4xl font-medium">Comic Website Catalog</h1>
</div>
<div class="space-y-6 bg-white p-8 max-sm:p-4">
	{#if totalComic > 0}
		<div class="flex flex-wrap justify-center gap-4 py-2">
			{#each comics as comic}<CardComic {comic} comicData={data.comicData} />{/each}
		</div>
