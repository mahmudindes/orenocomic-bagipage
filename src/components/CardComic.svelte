<script lang="ts">
	import type { BCComic, DEComic } from '$lib/api';
	import { onMount } from 'svelte';
	import { getComicCover, getComicTitle } from '$lib/helper';
	import loadingAnimation from '$assets/loadingAnimation.gif';

	export let comic: BCComic;
	export let comicData: (code: string) => Promise<DEComic>;

	let cover: string = loadingAnimation;
	let title: string = 'Loading Title...';
	onMount(async () => {
		const data = await comicData(comic.code);
		cover = getComicCover(data);
		title = getComicTitle(data, 'en');
	});
</script>
